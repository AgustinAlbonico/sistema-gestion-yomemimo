...
export function exportTopProductsToCSV(products: TopProduct[], filename?: string): void {
    const rows = [
        ...
        [''],
        ['Producto', 'SKU', 'Cantidad Vendida', 'Ingresos', 'Costo', 'Ganancia', 'Margen %', 'Tipo'],
        ...products.map(p => [
            p.productName,
            p.productSku || '-',
            ...
            p.hasKnownCost ? 'Con margen' : 'Precio Fijo',
        ]),
    ];
...
export function exportProductsToCSV(report: ProductsReport, filename?: string): void {
    ...
    const rows = [
        ...
        [''],
        ['=== TOP PRODUCTOS ==='],
        ['Producto', 'SKU', 'Cantidad', 'Ingresos', 'Ganancia', 'Margen %', 'Tipo'],
