...
/**
 * Busca usuario incluyendo el passwordHash (excluido por defecto)
 */
async function fetchUserWithPasswordHash(
...
async function validateAndFixPassword(
    ...
    const updatedUser = await fetchUserWithPasswordHash(userRepository, { id: user.id });
...
(async () => {
    try {
        ...
        const user = await fetchUserWithPasswordHash(userRepository, { username });
        ...
        if (!user) {
            ...
            const success = await validateAndFixPassword(userRepository, user, testPassword);
